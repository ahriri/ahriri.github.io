...
/* WEBPACK VAR INJECTION */(function(jQuery) {(function ($) {
  ...
  window.addEventListener('load', function () {
    ...
    const svgsLen = svgs.length; // Loop and process.
    ...
    for (let i = 0; i < svgsLen; ++i) {
      ...
      svgs[i].removeAttribute('data-cb'); // We'll let another function handle the actual fetching
  ...
  const fetchSVG = async function (url, el, cb) {
    ...
    let data = await response.text(); // This response should be an XML document we can parse.

    const parser = new DOMParser();
    const parsed = parser.parseFromString(data, 'image/svg+xml'); // The file might not actually begin with "<svg>", and
  ...
  $(window).scroll(function () {
    ...
    if (lastId !== id) {
      ...
      menuItems.parent().removeClass("active").blur().end().filter("[href='#" + id + "']").parent().addClass("active");
    }
  ...
  $(document).ready(function () {
    ...
    (function ($) {
      var owl = $("#owl-roadmap"); // owl.owlCarousel();
